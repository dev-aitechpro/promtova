...
const canonicalizePrompts = (prompts: Prompt[], folders: Folder[]): Prompt[] => prompts.map((prompt) => {
  const folder = (prompt.folderId && folders.find((item) => item.id === prompt.folderId)) ?? folders.find((item) => item.name === prompt.folder);
  return {
    ...prompt,
    folderId: folder?.id,
...
const promptStore = create<PromptStoreState>()(
  persist(
    (set, get) => ({
      ...
      createPrompt: (folderInput = 'Development') => {
        const state = get();
        const target = resolveFolder(folderInput, state.folders) ?? state.folders[0];
        ...
        const prompt: Prompt = {
          ...
          folderId: target?.id,
          folder: target?.name ?? 'Development',
          vars: {},
          starred: false,
          createdAt: now,
          updatedAt: now,
        ...
        set((state) => ({ prompts: [prompt, ...state.prompts], tags: recomputeTags([prompt, ...state.prompts]), selectedPromptId: id, isDirty: false }));
        return id;
      },
      ...
      updatePrompt: (id, patch, versionNote = '') => set((state) => {
        ...
        const fingerprintBefore = JSON.stringify({ ...current, updatedAt: undefined });
        const fingerprintAfter = JSON.stringify({ ...next, updatedAt: undefined });
        if (fingerprintBefore === fingerprintAfter) return state;
      ...
      renameFolder: (id, newName) => {
        ...
        set((state) => {
          ...
          const prompts = state.prompts.map((prompt) => prompt.folderId === id || (!prompt.folderId && prompt.folder === state.folders.find((folder) => folder.id === id)?.name)
            ? { ...prompt, folderId: id, folder: clean, path: `${clean}/${prompt.title}`, updatedAt: new Date().toISOString() }
            : prompt);
      ...
      deleteFolder: (id) => set((state) => {
        ...
        return {
          ...
          prompts,
          tags: recomputeTags(prompts),
          versions: state.versions.filter((version) => kept.has(version.promptId)),
          runs: state.runs.filter((run) => kept.has(run.promptId)),
      ...
      moveFolderUp: (id) => set((state) => {
        ...
        const orderMap = new Map(ordered.map((folder, i) => [folder.id, i]));
      ...
      movePromptToFolder: (id, folderInput) => {
        const folder = resolveFolder(folderInput, get().folders);
        if (folder) get().updatePrompt(id, { folderId: folder.id }, 'Move prompt to folder');
      },
      ...
      restoreVersion: (promptId, versionId, note = 'Restore version') => {
        ...
        const version = state.versions.find((item) => item.id === versionId && item.promptId === promptId);
        ...
        const prompts = state.prompts.map((item) => item.id === promptId ? canonicalizePrompts([restored], state.folders)[0] : item);
        set((current) => ({ prompts, versions: [...current.versions, backup], tags: recomputeTags(prompts), isDirty: false, lastSavedAt: new Date().toISOString() }));
        return prompts.find((item) => item.id === promptId) ?? null;
      },
      ...
      applyTemplateToPrompt: (promptId, templateId, versionNote = 'Apply template') => { const template = get().templates.find((item) => item.id === templateId); if (!template) return; get().updatePrompt(promptId, { templateId, sections: template.sections.map((section) => ({ ...section })), useTemplate: true }, versionNote); },
      ...
      addBlockToPrompt: (promptId, blockId, overrides) => { const prompt = get().prompts.find((item) => item.id === promptId); if (!prompt || !get().blocks.some((block) => block.id === blockId)) return; const refs = [...(prompt.blockRefs ?? [])].filter((reference) => reference.blockId !== blockId); refs.push({ blockId, order: refs.length, overrides }); get().updatePrompt(promptId, { blockRefs: refs }, 'Add prompt block'); },
      ...
      addModelProfile: (name, modelProvider, model) => { const profile = createModelProfile(name, modelProvider, model); set((state) => ({ modelProfiles: [...state.modelProfiles, profile] })); return profile.id; },
      ...
      applyImport: (incoming, conflicts, incomingFolders, parsed) => {
        ...
        const sorted = [...incomingFolders].sort((a, b) => Number(Boolean(a.parent)) - Number(Boolean(b.parent)));
        let foldersCreated = 0;
        for (const source of sorted) {
          const existing = existingByName.get(source.name);
          if (existing) { folderIdMap.set(source.id, existing.id); continue; }
          const parentId = source.parent ? (folderIdMap.get(source.parent) ?? folders.find((folder) => folder.id === source.parent)?.id ?? null) : null;
          const created: Folder = { ...source, id: source.id && !folders.some((folder) => folder.id === source.id) ? source.id : newId(), parent: parentId, children: [], order: getSiblings(folders, parentId).length };
        ...
        const normalizedConflicts: MergeConflict[] = conflicts.map((conflict) => ({ ...conflict, key: conflictKey(normalizedIncoming.find((item) => item.id === conflict.incoming.id) ?? conflict.incoming), incoming: normalizedIncoming.find((item) => item.id === conflict.incoming.id) ?? conflict.incoming, existing: normalizedExisting.find((item) => item.id === conflict.existing.id) ?? conflict.existing }));
        const merged = applyMerge(normalizedExisting, normalizedIncoming, normalizedConflicts);
        const idMap = new Map(Object.entries(merged.promptIdMap));
        const addEntities = <T extends { id: string }>(current: T[], values: T[]): T[] => {
          const result = [...current]; const ids = new Set(result.map((item) => item.id));
          values.forEach((value) => { if (!ids.has(value.id)) { result.push(value); ids.add(value.id); } });
          return result;
        };
        const versions = addEntities(state.versions, (parsed?.versions ?? []).flatMap((version) => { const mapped = idMap.get(version.promptId); return mapped ? [{ ...version, promptId: mapped }] : []; }));
        const runs = addEntities(state.runs, (parsed?.runs ?? []).flatMap((run) => { const mapped = idMap.get(run.promptId); return mapped ? [{ ...run, promptId: mapped }] : []; }));
        const templates = addEntities(state.templates, parsed?.templates ?? []);
        const blocks = addEntities(state.blocks, parsed?.blocks ?? []);
        const modelProfiles = addEntities(state.modelProfiles, parsed?.modelProfiles ?? []);
    ...
    }),
    {
      ...
      migrate: (persisted) => {
        ...
        const folders = normalizeFolders(
          (Array.isArray(raw.folders) ? raw.folders : []).map(normalizeFolder).filter((item): item is Folder => item !== null),
        );
        const usableFolders = folders.length ? folders : initialFolders;
        const prompts = canonicalizePrompts(
          (Array.isArray(raw.prompts) ? raw.prompts : []).map((item) => normalizePrompt(item)).filter((item): item is Prompt => item !== null),
          usableFolders,
        );
        ...
        const selectedFolderId = selectedRaw === 'all' || selectedRaw === 'starred' || usableFolders.some((folder) => folder.id === selectedRaw)
          ? selectedRaw
          : usableFolders.find((folder) => folder.name === selectedRaw)?.id ?? 'all';
        return {
          ...
          modelProfiles: Array.isArray(raw.modelProfiles) ? raw.modelProfiles : [],
