...
export interface PromptStoreState {
  ...
  exportData: () => Pick<ParsedImport, 'prompts' | 'folders' | 'versions' | 'templates' | 'blocks' | 'modelProfiles' | 'runs'>;
}
...
const createStoreState = (set: any, get: any): PromptStoreState => {
  const commitUpdate = (id: PromptId, patch: Partial<Prompt>, versionNote = '') => {
    set((state: PromptStoreState) => {
      ...
      // One snapshot per dirty editing session: the version records the state before the first change.
      const hasDraftBaseline = state.versions.some((version) =>
        version.promptId === id && version.createdAt === state.lastSavedAt,
      );
      const version = hasDraftBaseline ? null : createPromptVersion(current, nextPromptVersion(state.versions, id), versionNote);
      return {
  ...
  return {
    ...
    createPrompt: (folderInput = 'Development') => {
      const state = get();
      const target = state.folders.find((folder: Folder) => folder.id === folderInput) ?? state.folders.find((folder: Folder) => folder.name === folderInput) ?? state.folders[0];
      const now = new Date().toISOString();
      const prompt: Prompt = {
        id: newId(), title: 'Новый промпт', tags: [], preview: '', path: `${target?.name ?? 'Development'}/Новый промпт`,
      ...
      set((state: PromptStoreState) => ({ prompts: [prompt, ...state.prompts], selectedPromptId: prompt.id, tags: recomputeTags([prompt, ...state.prompts]) }));
      return prompt.id;
    },
    ...
    duplicatePrompt: (id) => {
      ...
      const now = new Date().toISOString();
      copy.id = newId(); copy.title = `${source.title} (копия)`; copy.createdAt = now; copy.updatedAt = now; copy.usageCount = 0; copy.starred = false;
      set((state: PromptStoreState) => ({ prompts: [copy, ...state.prompts], tags: recomputeTags([copy, ...state.prompts]), selectedPromptId: copy.id }));
    },

    renamePrompt: (id, title) => {
      const prompt = get().prompts.find((item: Prompt) => item.id === id);
      if (!prompt) return;
      commitUpdate(id, { title: title.trim() }, 'Rename prompt');
    },

    toggleStar: (id) => {
      const prompt = get().prompts.find((item: Prompt) => item.id === id);
      if (prompt) commitUpdate(id, { starred: !prompt.starred }, 'Toggle star');
    },

    incrementUsage: (id) => {
      const prompt = get().prompts.find((item: Prompt) => item.id === id);
      if (prompt) commitUpdate(id, { usageCount: prompt.usageCount + 1 }, 'Usage increment');
    },

    setVar: (id, key, value) => {
      const prompt = get().prompts.find((item: Prompt) => item.id === id);
      if (prompt) commitUpdate(id, { vars: { ...prompt.vars, [key]: value } }, 'Update variable');
    },
    ...
    renameFolder: (id, newName) => {
      ...
      set((state: PromptStoreState) => {
        ...
        const folders = state.folders.map((item: Folder) => item.id === id ? { ...item, name: clean } : item);
        return { folders, prompts, tags: recomputeTags(prompts) };
      });
    },

    deleteFolder: (id) => set((state: PromptStoreState) => {
      const removedIds = new Set([id, ...getDescendantIds(state.folders, id)]);
      const folders = normalizeFolders(state.folders.filter((folder: Folder) => !removedIds.has(folder.id)).map((folder: Folder) => ({ ...folder, children: folder.children.filter((child) => !removedIds.has(child)) })));
      const prompts = canonicalizePrompts(state.prompts, state.folders).filter((prompt) => !removedIds.has(prompt.folderId ?? ''));
      const keptPromptIds = new Set(prompts.map((prompt) => prompt.id));
      return {
        folders, prompts, tags: recomputeTags(prompts),
        versions: state.versions.filter((version) => keptPromptIds.has(version.promptId)),
        runs: state.runs.filter((run) => keptPromptIds.has(run.promptId)),
        selectedFolderId: removedIds.has(state.selectedFolderId) ? 'all' : state.selectedFolderId,
        selectedPromptId: prompts.some((prompt) => prompt.id === state.selectedPromptId) ? state.selectedPromptId : (prompts[0]?.id ?? null),
      };
    }),
    ...
    countFolderPrompts: (id) => {
      const state = get(); const ids = new Set([id, ...getDescendantIds(state.folders, id)]); return canonicalizePrompts(state.prompts, state.folders).filter((prompt) => ids.has(prompt.folderId ?? '')).length;
    },

    movePromptToFolder: (id, folderInput) => {
      const folderId = folderIdFromInput(folderInput, get().folders);
    ...
    markSaved: () => set((state: PromptStoreState) => ({ isDirty: false, lastSavedAt: new Date().toISOString(), _clearDraftBaseline: newId() } as Partial<PromptStoreState>)),
    setAutosave: (v) => set({ autosave: v }),
    setEditorFontSize: (v) => set({ editorFontSize: Math.min(20, Math.max(10, Math.round(v))) }),
    ...
    restoreVersion: (promptId, versionId, note = 'Restore version') => {
      const state = get(); const prompt = state.prompts.find((item: Prompt) => item.id === promptId); const version = state.versions.find((item: PromptVersion) => item.id === versionId && item.promptId === promptId);
      ...
      set((current: PromptStoreState) => ({ prompts: current.prompts.map((item) => item.id === promptId ? restored : item), versions: [...current.versions, snapshot], tags: recomputeTags(current.prompts.map((item) => item.id === promptId ? restored : item)), isDirty: false, lastSavedAt: new Date().toISOString() }));
      return restored;
    },
    ...
    applyTemplateToPrompt: (promptId, templateId, versionNote = 'Apply template') => {
      const state = get(); const template = state.templates.find((item: PromptTemplate) => item.id === templateId); if (!template) return;
      commitUpdate(promptId, { templateId, sections: template.sections.map((s) => ({ ...s })), useTemplate: true }, versionNote);
    },
    ...
    addBlockToPrompt: (promptId, blockId, overrides) => {
      const prompt = get().prompts.find((item: Prompt) => item.id === promptId); if (!prompt || !get().blocks.some((block: PromptBlock) => block.id === blockId)) return;
      const refs = [...(prompt.blockRefs ?? [])].filter((ref) => ref.blockId !== blockId);
      refs.push({ blockId, order: refs.length, overrides });
      commitUpdate(promptId, { blockRefs: refs }, 'Add prompt block');
    },
    removeBlockFromPrompt: (promptId, blockId) => {
      const prompt = get().prompts.find((item: Prompt) => item.id === promptId); if (!prompt) return;
      const refs = (prompt.blockRefs ?? []).filter((ref) => ref.blockId !== blockId).map((ref, index) => ({ ...ref, order: index }));
      commitUpdate(promptId, { blockRefs: refs }, 'Remove prompt block');
    },
    ...
    recordRun: (promptId, options = {}) => { const prompt = get().prompts.find((item: Prompt) => item.id === promptId); if (!prompt) return null; const run = createPromptRun(prompt, { ...options, versionId: options.versionId ?? get().versions.filter((v: PromptVersion) => v.promptId === promptId).at(-1)?.id }); set((state: PromptStoreState) => ({ runs: [run, ...state.runs] })); return run.id; },
    ...
    applyImport: (incoming, conflicts, incomingFolders, parsed) => {
      ...
      const existingByNameParent = new Map(folders.map((folder) => [`${folder.parent ?? 'root'}\u0000${folder.name}`, folder]));
      let foldersCreated = 0;

      for (const source of incomingFolders) {
        const key = `${source.parent ?? 'root'}\u0000${source.name}`;
        const existing = existingByNameParent.get(key);
        ...
        folderIdMap.set(source.id, created.id);
        existingByNameParent.set(`${parentId ?? 'root'}\u0000${created.name}`, created);
        foldersCreated++;
      }

      const normalizedIncoming = canonicalizePrompts(incoming.map((prompt) => ({ ...prompt, folderId: prompt.folderId ? (folderIdMap.get(prompt.folderId) ?? prompt.folderId) : prompt.folderId })), folders);
      const normalizedExisting = canonicalizePrompts(state.prompts, folders);
      const merged = applyMerge(normalizedExisting, normalizedIncoming, conflicts);
      const keep = new Set(merged.prompts.map((prompt) => prompt.id));
      const normalizedParsed = parsed ?? null;
      const addUnique = <T extends { id: string }>(current: T[], values: T[]) => {
        const result = [...current]; const ids = new Set(current.map((item) => item.id));
        for (const value of values) if (!ids.has(value.id)) { result.push(value); ids.add(value.id); }
        return result;
      };

      const versions = addUnique(state.versions, (normalizedParsed?.versions ?? []).filter((version) => keep.has(version.promptId)));
      ...
      set({ prompts: merged.prompts, folders: normalizeFolders(folders), versions, templates, blocks, modelProfiles, runs, tags: recomputeTags(merged.prompts) });
    ...
    exportData: () => {
      ...
      return {
        ...
        versions: state.versions.map((version) => ({ ...version, sections: version.sections.map((s) => ({ ...s })), variables: version.variables.map((v) => ({ ...v })), legacy: { ...version.legacy } })),
        templates: state.templates.map((template) => ({ ...template, sections: template.sections.map((s) => ({ ...s })) })),
        blocks: state.blocks.map((block) => ({ ...block, tags: [...block.tags], variables: block.variables.map((v) => ({ ...v })) })),
        modelProfiles: state.modelProfiles.map((profile) => ({ ...profile, capabilities: profile.capabilities ? [...profile.capabilities] : undefined, params: profile.params ? { ...profile.params } : undefined })),
        runs: state.runs.map((run) => ({ ...run, input: { ...run.input }, criteria: run.criteria.map((c) => ({ ...c })), tokenUsage: run.tokenUsage ? { ...run.tokenUsage } : undefined })),
      };
    },
  };
};
...
export const usePromtovaStore = create<PromptStoreState>()(
  persist((set, get) => createStoreState(set, get), {
    ...
    migrate: (persisted, version) => {
      ...
      const folders = normalizeFolders((Array.isArray(raw.folders) ? raw.folders : []).map(normalizeFolder).filter((x): x is Folder => x !== null));
      const usableFolders = folders.length ? folders : initialFolders;
      const prompts = canonicalizePrompts(
        (Array.isArray(raw.prompts) ? raw.prompts : []).map((item) => normalizePrompt(item)).filter((x): x is Prompt => x !== null),
        usableFolders,
      );
      ...
      if (selectedFolderId !== 'all' && selectedFolderId !== 'starred' && !usableFolders.some((folder) => folder.id === selectedFolderId)) {
        selectedFolderId = usableFolders.find((folder) => folder.name === selectedFolderId)?.id ?? 'all';
      }
      return {
        ...
        blocks: Array.isArray(raw.blocks) ? raw.blocks as PromptBlock[] : [],
...
export const applyTheme = (themeId: string, customThemes: CustomTheme[] = []) => {
  ...
  if (themeId.startsWith('custom-')) {
    ...
    if (theme) Object.entries(theme.colors).forEach(([k, v]) => root.style.setProperty(`--${k}`, v));
    return;
  }
  const preset = presetThemes[themeId];
  if (preset) { Object.entries(preset).forEach(([k, v]) => root.style.setProperty(`--${k}`, v)); return; }
  ['bg-primary','bg-sidebar','bg-panel','bg-elevated','bg-hover','bg-active','accent-primary','accent-hover','accent-subtle','text-primary','text-secondary','text-muted','border-primary','border-subtle'].forEach((v) => root.style.removeProperty(`--${v}`));
};
...
interface UIState {
  ...
  folderModalParentId: string | null; renameFolderId: string | null; renamePromptId: PromptId | null; deleteFolderId: string | null; mergeImport: ParsedImport | null;
  toasts: Toast[];
  openSettings: () => void; closeSettings: () => void; openExport: () => void; closeExport: () => void;
  openFolderModal: (parentId?: string | null) => void; closeFolderModal: () => void; openTagModal: () => void; closeTagModal: () => void;
  openThemeEditor: () => void; closeThemeEditor: () => void; openShortcuts: () => void; closeShortcuts: () => void;
  openRenameFolder: (id: string) => void; closeRenameFolder: () => void; openRenamePrompt: (id: PromptId) => void; closeRenamePrompt: () => void;
  openDeleteFolder: (id: string) => void; closeDeleteFolder: () => void; openMerge: (data: ParsedImport) => void; closeMerge: () => void;
  pushToast: (t: Omit<Toast, 'id'>) => void; dismissToast: (id: number) => void;
}
let toastCounter = 0;
export const useUIStore = create<UIState>((set) => ({
  ...
  pushToast:(t)=>{ const id=++toastCounter; set((s)=>({toasts:[...s.toasts,{id,...t}]})); setTimeout(()=>set((s)=>({toasts:s.toasts.filter((x)=>x.id!==id)})),3200); },
  dismissToast:(id)=>set((s)=>({toasts:s.toasts.filter((x)=>x.id!==id)})),
}));

export const useThemeStore = create<ThemeState>()(persist((set)=>({
  currentTheme:'dark', customThemes:[], setTheme:(id)=>set({currentTheme:id}), addCustomTheme:(theme)=>set((s)=>({customThemes:[...s.customThemes,theme]})),
  removeCustomTheme:(id)=>set((s)=>({customThemes:s.customThemes.filter((t)=>t.id!==id),currentTheme:s.currentTheme===id?'dark':s.currentTheme})),
